...
public class XuatXuDialog extends JDialog implements MouseListener {
    ...
    private IOriginHandle br= new OriginHandle();
    List<Origin> list = br.findAll();
    ...
    public void loadDataTable(List<Origin> result) {
        tblModel.setRowCount(0);
        for (Origin ncc : result) {
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == add) {
            if (Validation.isEmpty(ms.getText())) {
                ...
                if (br.checkUp(tenmau)) {
                    br.addPhone(new ListTransfer(new Ram(1,tenmau ,1),"add"));
                    loadDataTable(list);
                    ms.setText("");
                } else {
            ...
            if (index != -1) {
                br.deletePhone(new ListTransfer(list.get(index),"delete"));
                loadDataTable(list);
                ms.setText("");
            }
            ...
            if (index != -1) {
                if (Validation.isEmpty(ms.getText())) {
                    ...
                    String tenmau = ms.getText();
                    if (br.checkUp(tenmau)) {
                        br.editPhone(new ListTransfer(new Ram(list.get(index).getId(), tenmau,1),"edit"));
                        loadDataTable(list);
                        ms.setText("");
                    } else {
                        JOptionPane.showMessageDialog(this, "Xuất xứ đã tồn tại !");
